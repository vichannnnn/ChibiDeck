...
| Right (1640 px) | Every session, four cards to a row, two rows in view: status and elapsed time, the session's title (Claude Code's own title, else its name), `repo@branch`, Claude's last message, the task list, the context-window bar, model and effort chips, and a one-tap answer pill. Sessions waiting on you come first (waiting, blocked, idle), then the ones in progress (working, shell). Colour marks what needs you: waiting and blocked cards get a bold word and a border in their status colour (yellow, red); idle gets the dot and word in the theme accent, with a quiet border; working and shell keep their status colour on the dot only — the word and border stay quiet grey. |
